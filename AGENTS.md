...
| `/docs-sync` | Сверка документации `docs/*.md` и `AGENTS.md` с фактическим состоянием кодовой базы (пути, строки, структура) |
| `/task {N}`  | Выполнение задачи N из `docs/feature-branch/implementation-plan.md` — планирование, реализация, отчёт       |

## GitHub Project

Проект [`resource_lake_refactoring`](https://github.com/users/VyacheslavBogdanov/projects/2) (номер 2) используется для трекинга задач. **Codex выполняет эти действия автоматически** — пользователю не нужно запускать команды вручную.
...
**Когда:** Codex создаёт ветку под задачу.
