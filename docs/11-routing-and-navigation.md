...
3. **Нет lazy-loading** — все 4 страницы (4358 строк суммарно) загружаются при старте
4. **ResourcePlan.vue (2432 строки)** особенно тяжёлый для eager-загрузки
...
- `ResourcePlan-[hash].js` (~2432 строки компонента)
- `Projects-[hash].js` (~688 строк)
- `Groups-[hash].js` (~507 строк)
- `DataManage-[hash].js` (~731 строк)
