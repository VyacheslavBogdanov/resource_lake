...
3. **Нет lazy-loading** — все 4 страницы (3597 строк суммарно) загружаются при старте
4. **ResourcePlan.vue (1982 строки)** особенно тяжёлый для eager-загрузки
...
- `ResourcePlan-[hash].js` (~1982 строки компонента)
- `Projects-[hash].js` (~640 строк)
- `Groups-[hash].js` (~484 строки)
- `DataManage-[hash].js` (~491 строка)
